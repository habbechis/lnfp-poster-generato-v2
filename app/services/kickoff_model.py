...
def _bg(background=None):
    ...
    if background:
        ...
        # The UI may still send the old default filename; in that case prefer
        # the supplied reference asset instead.
...
def _glass_card(im, box):
    ...
    # Slightly translucent crystal white, close to the supplied model.
...
def _row(im, match, y0, y1):
    ...
    cy = (y0 + y1) / 2
    rh = y1 - y0
    # Larger crests than the previous version, matching the model's visual weight.
    ...
    if an:
        ...
        p._draw_text(d, (485, cy), an, fs, "Bold", anchor="lm", rtl=True, fill=TEXT)
...
def render_kickoff(matchweek, days: list[dict], brand_logo=None, background=None, scale=1.0):
    im = _bg(background)
    d = ImageDraw.Draw(im)
    # Header enlarged to match the supplied reference model while remaining inside 4:5.
    try:
    ...
    # Keep all dynamic fixtures inside the native 2000x2500 canvas.
    top, bottom = 600, 2410
    date_h, gap = 94, 26
    row_h = 166
