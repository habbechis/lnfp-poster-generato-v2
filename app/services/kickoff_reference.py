...
def _card(base, box, radius=28):
    ...
    d.rounded_rectangle((x0, y0, x1, y1), radius=radius, fill=(255, 255, 255, 250),
                        outline=BORDER, width=5)
    # Very subtle inner highlight, so the card reads as a clean premium panel.
    d.rounded_rectangle((x0 + 7, y0 + 7, x1 - 7, y1 - 7), radius=max(8, radius - 7),
                        outline=(255, 255, 255, 230), width=2)


def _date_icon(base, cx, cy, size):
    """Simple white calendar glyph; avoids depending on a particular icon asset."""
    d = ImageDraw.Draw(base)
    s = int(size)
...
def _date_badge(base, text, cy, width=560, height=104):
    ...
    size = _p._fit_font(draw, text, width - 145, int(height * .48), "Bold", min_size=24,
                        rtl=True)
    _p._draw_text(draw, (cx + 24, cy), text, size, "Bold", fill=WHITE, anchor="mm")


def _time_plate(base, cx, cy, text, height):
    """Wide angular red time plate, closer to the sports-card reference than a pill."""
    d = ImageDraw.Draw(base)
    h = int(height)
    ...
    poly = [(x0 + slant, y0), (x1 - slant, y0), (x1, y0 + h / 2),
            (x1 - slant, y1), (x0 + slant, y1), (x0, y0 + h / 2)]
    d.polygon(poly, fill=RED)
    d.line(poly + [poly[0]], fill=(255, 255, 255, 105), width=3)
    d.line((x0 + slant + 18, y0 + 10, x1 - slant - 18, y0 + 10),
           fill=(255, 255, 255, 80), width=3)
    size = _p._fit_font(d, text, width - 80, int(h * .52), "ExtraBold", min_size=28,
                        rtl=False, role="time")
...
def _match_row(base, match, y0, y1):
    ...
    # RTL composition: home team on the right, away team on the left.
    ...
    if home_name:
        ...
        _p._draw_text(draw, (home_logo_x - logo_box * .72, mid), home_name, name_size_h,
                      "Bold", fill=TEXT, anchor="rm")
    if away_name:
        name_size_a = _p._fit_font(draw, away_name, 420, name_size, "Bold", min_size=24)
        _p._draw_text(draw, (away_logo_x + logo_box * .72, mid), away_name, name_size_a,
                      "Bold", fill=TEXT, anchor="lm")

    _time_plate(base, center, mid, (match.get("time") or "16:30").strip(), int(min(112, h * .48)))

    # Fine separators visually connect the teams to the central time plate.
    d = ImageDraw.Draw(base)
...
def render_kickoff(matchweek, days: list[dict], brand_logo: str | None = None,
    ...
    # Header geometry follows the reference: centered league mark, dominant title,
    # then a compact matchweek label before the first date badge.
    logo_cy = 150
    _p._paste_brand_logo(base, W / 2, logo_cy, 190, 190, spec=brand_logo)
    draw = ImageDraw.Draw(base)
    _p._draw_text(draw, (W / 2, 320), "KICK OFF", 142, "ExtraBold",
    ...
    _p._draw_text(draw, (W / 2, 435), label, 60, "Bold", fill=_p.CRYSTAL,
                  anchor="mm", rtl=False, role="time")
    ...
    # Reference target is intentionally airy.  Calculate a stable row height first,
    # then scale only when a large fixture list would otherwise overflow.
    top = 560
    bottom = 2390
    day_gap = 34
    date_h = 104
    row_h = 245
    row_gap = 18
    total = sum(date_h + len(d["matches"]) * row_h + max(0, len(d["matches"]) - 1) * row_gap
                for d in days) + max(0, len(days) - 1) * day_gap
    if total > bottom - top:
